...
private class UserRepositoryImpl(
    ...
    override fun getPaginatedList(scope: CoroutineScope, token: String): LiveData<PagedList<User>> {
        return LivePagedListBuilder(
            UserDataSource.Factory(api, scope, token),
            paginationConfig
        ).build()
    }

    override fun getSearchPaginatedList(scope: CoroutineScope, query: String, sort: String, token: String): LiveData<PagedList<User>> {
        return  LivePagedListBuilder(
            SearchUserDataSource.Factory(api, scope, query, sort, token),
            paginationConfig
        ).build()
    }

    override suspend fun getUserDetails(url: String, token: String): User? {
        return withContext(Dispatchers.IO) {
            try {
                val response = api.getUserDetails(token, url)
...
interface UserRepository {
     ...
     */
    fun getPaginatedList(scope: CoroutineScope, token: String): LiveData<PagedList<User>>

    /**
     * Return a LiveData used in the search feature
     */
    fun getSearchPaginatedList(scope: CoroutineScope, query: String, sort: String, token: String): LiveData<PagedList<User>>

    suspend fun getUserDetails(url: String, token: String): User?
